...
class Select_model extends CI_Model
{
    function cekTanggal()
    {
        $this->db->select('curdate() + interval 5 year as max_tanggal');
        $query_min = $this->db->get();
        return $query_min->row_array();
    }
    ...
    function getDataAllBaru()
    {
        ...
        $query  = $this->db->where('D.status_rekomendasi', 'KEPALA');
        $query  = $this->db->get();
        return $query->result();
    }
    function getDataAllPerpanjang()
    {
        ...
        $query  = $this->db->join('tbl_rekomendasi as D', 'A.id_user=D.id_user');
        $query  = $this->db->join('tbl_kategori as E', 'E.id_kategori=D.id_kategori');
        $query  = $this->db->where('D.status_rekomendasi', 'P_KABID');
        $query  = $this->db->or_where('D.status_rekomendasi', 'P_KEPALA');
        $query  = $this->db->get();
        return $query->result();
    }
    ...
    function getDataValidasi()
    {
        ...
        $query  = $this->db->where('A.status_sip', 'KABID');
        $query  = $this->db->or_where('A.status_sip', 'P_KABID');
        $query  = $this->db->get();
        return $query->result();
    }
